...
public:
  ...
  : Node("go2_cmd_vel_bridge")
  {
    ...
    auto_running_mode_ = declare_parameter<bool>("auto_running_mode_on_start", false);
    auto_classic_mode_ = declare_parameter<bool>("auto_classic_mode_on_start", false);
    ...
    if (auto_balance_stand_ || auto_running_mode_ || auto_classic_mode_) {
      startup_timer_ = create_wall_timer(std::chrono::milliseconds(500), [this]() {
        if (auto_balance_stand_ && !startup_balance_sent_) {
          publish_motion_request(kApiBalanceStand);
          startup_balance_sent_ = true;
          RCLCPP_INFO(get_logger(), "Sent BalanceStand request to GO2.");
          return;
        }

        if (auto_classic_mode_) {
          publish_bool_request(kApiClassicWalk, true);
          RCLCPP_INFO(
            get_logger(),
            "Sent ClassicWalk request to GO2 (Classic Mode)."
          );
        } else if (auto_running_mode_) {
          publish_motion_request(kApiTrotRun);
          RCLCPP_INFO(
            get_logger(),
            "Sent TrotRun request to GO2 (Running Mode, equivalent to L2 + START).");
        }
...
private:
  ...
  bool dispatch_named_command(const go2_interface::msg::SportCmd & msg)
  {
    ...
    if (command == "running_mode" || command == "trot_run") {
      publish_motion_request(kApiTrotRun);
      return true;
    }
  ...
  bool auto_running_mode_{false};
  bool auto_classic_mode_{false};
  bool startup_balance_sent_{false};
