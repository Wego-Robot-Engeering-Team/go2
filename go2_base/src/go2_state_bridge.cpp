...
public:
  ...
  : Node("go2_state_bridge")
  {
    ...
    planarize_odom_ = declare_parameter<bool>("planarize_odom", false);
...
private:
  void on_sport_state(const unitree_go::msg::SportModeState::SharedPtr msg)
  {
    ...
    // AMCL, SLAM Toolbox, and Nav2 operate in SE(2). Feeding the Go2 body
    // roll/pitch/height into odom -> base_link makes map -> odom compensate
    // those body motions and causes the 2D localization frame to wobble.
    // Keep the full attitude on /go2/imu and publish planar odometry here.
    if (planarize_odom_) {
      const double yaw = quaternion_yaw(odom.pose.pose.orientation);
